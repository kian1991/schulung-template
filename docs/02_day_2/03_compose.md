...
```yaml
version: '3.8'  # Optional in neueren Versionen
...
| Befehl | Was passiert? |
| :--- | :--- |
| `docker compose up` | Baut Images, erstellt Netzwerke, Volumes und startet alle Container (Vordergrund). |
| `docker compose up -d` | Das Gleiche, aber im Hintergrund (Detached). |
| `docker compose down` | Stoppt und **löscht** Container und Netzwerke. (Volumes bleiben!). |
| `docker compose down -v` | Löscht AUCH die Volumes (Alles weg!). |
| `docker compose ps` | Zeigt Status des Stacks. |
| `docker compose logs -f` | Zeigt Logs aller Services aggregiert. |
